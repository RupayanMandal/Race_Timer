...
class race():
    ...
    def set_frame(self):    
        ...
        add_btn=customtkinter.CTkButton(master=self.frame,height=100,text='ADD TEAM',font=('courier',18,'bold'),state='normal',command=lambda: self.add_teams_checkbox(add_btn,checkbox_frame))
    ...
    def add_teams_checkbox(self,add_btn,checkbox_frame):
        ...
        if race_type=='TIME TRIAL':
            time_trial.time_trial_race(number_of_laps=3,participant_name_list=rfid_list)
        elif race_type=='FORMULA':
            formula.formula_race(6,rfid_list)    
