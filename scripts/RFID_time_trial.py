...
def scoreboardcalculation(lock,path,quit_val,log_lock,start_time,number_of_laps,com_port: int):
    ...
    while True:
        ...
        try:
            ...
            pass    
        if sum(list(map(itemgetter('lap'), data)))==len(data)*(number_of_laps):
            msg="Race ENDED"
            log_lock.acquire()
            my_log.write(msg)
            my_log.savelogfile()
...
class time_trial_race():
    def __init__(self,com_port,number_of_laps,participant_name_list: list):
        ...
        p1 = multiprocessing.Process(target=scoreboardcalculation,args=(lock,path,quit_val,log_lock,start_time,number_of_laps,com_port))
...
if __name__=='__main__':
    time_trial_race(6,2,[83150223167,969414427])        