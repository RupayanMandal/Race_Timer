...
def scoreboardcalculation(lock,path,quit_val,log_lock,start_time,number_of_laps,playerid):
    ...
    while not quit_val.value:
        ...
        playerid.value = 0
        #print('iddddd: ',id)
        if quit_val.value or sum(list(map(itemgetter('lap'), data)))==len(data)*(number_of_laps):
            msg="Race ENDED"
            log_lock.acquire()
            my_log.write(msg)
            my_log.savelogfile()
            ...
            dialog = customtkinter.CTkInputDialog(text="Enter file name:", title="Save result as")
            text = dialog.get_input()  # waits for input
            print(text)
            if text=='' or text==None:
                text=str(datetime.datetime.now()).replace(':','-')
            final_path='result_{}.txt'.format(text)
            with open(final_path,'w+') as new, open(path,'r') as old:
                for line in old:
                    new.write(line)

            break
        elif id==0 or (id not in list(map(itemgetter('id'),data)) and id!=0):
            continue 
        else:
            if not paused:
...
def scoreboarddisplay(lock,path,quit_val,log_lock,start_time,number_of_participants,number_of_laps):    
    ...
    def log_display(text_widget,log_lock):
        ...
        text_widget.insert(tkinter.END,msg) 
        text_widget.yview(tkinter.END)
        #text_widget.config(state='disabled')   

    def combobox_callback(choice,entry_widget):
        print("combobox dropdown clicked:", choice)
        entry_widget.configure(state='normal')

    def admin_button_callback(entry_widget,combobox_widget):
        try:
            id=int(entry_widget.get())
            choice=combobox_widget.get()
            confirm=tkinter.messagebox.askokcancel(title='ADMIN CONTROL',message='Do You want to {} team with ID {}'.format(choice,id))
            if confirm:
                #do something
                pass
    ...
    race_log=customtkinter.CTkTextbox(info_label,fg_color='transparent',text_color='red',
                        font=('Courier',10), wrap='word',
                        xscrollcommand=False,yscrollcommand=False,
                        )
        #3 Admin options
    admin_combo = customtkinter.CTkComboBox(master=display_label, values=["option 1", "option 2"],
                                     command=lambda choice: combobox_callback(choice,admin_entry))
    
    admin_entry = customtkinter.CTkEntry(master=display_label, state='disabled')

    admin_button = customtkinter.CTkButton(master=display_label, text='Proceed',
                                     command=lambda: admin_button_callback(admin_entry,admin_combo))
    ...
    admin_combo.grid(row=3,column=0,sticky='nsew',padx=10,pady=10) 
    admin_entry.grid(row=3,column=1,sticky='nsew',padx=10,pady=10)  
    admin_button.grid(row=4,column=1,sticky='nsew',padx=10,pady=10)     
    
    ## inside info label
    race_log.grid(row=0,column=0,sticky='nsew',padx=10,pady=10)
    ...
    tk_display(root,my_table_detail,my_table_rank,path,race_log,log_lock,elapsed_time,current_leader)
    #log_display(race_current_stat,log_lock)
        
    #root.after(1, tk_display, table, data)
    root.mainloop()    

def rfid_Scanner(listener,playerid,quit_val):
    while not quit_val.value:
        if listener==None:
            continue
        id=str(listener.readline())
...
def keyboard_scanner(playerid,quit_val):
    while not quit_val.value:
        i=keyboard.read_key()
        time.sleep(0.3)
        try:
...
class time_trial_race():
    def __init__(self,com_port,number_of_laps,participant_name_list: list, path: str = 'score.txt'):
        data = []
        ...
        try:
            ...
            listener=None
        playerid=multiprocessing.Value("l",0)
        processes = []

        p1 = multiprocessing.Process(target=scoreboardcalculation,args=(lock,path,quit_val,log_lock,start_time,number_of_laps,playerid))
        p2 = multiprocessing.Process(target=scoreboarddisplay,args=(lock,path,quit_val,log_lock,start_time,len(data),number_of_laps))
        p3 = multiprocessing.Process(target=keyboard_scanner,args=(playerid,quit_val))
        p4 = multiprocessing.Process(target=rfid_Scanner,args=(listener,playerid,quit_val))
        processes.append(p1)
        processes.append(p2)
        processes.append(p3)
        processes.append(p4)
...
if __name__=='__main__':
    time_trial_race(6,3,[1,83150223167,969414427],'')        